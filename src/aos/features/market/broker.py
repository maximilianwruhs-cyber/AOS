...
# Use centralized DATA_DIR from config (resolves to {PROJECT_ROOT}/data/)
from aos.config import DATA_DIR
DB_PATH = DATA_DIR / "aos_metrics.db"
...
def init_db():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # ── Schema migration: detect old schema and migrate data ──
        col_info = conn.execute("PRAGMA table_info(model_metrics)").fetchall()
        col_names = [c[1] for c in col_info]
        has_old = "avg_wattage" in col_names
        has_new = "avg_quality" in col_names

        if has_old and not has_new:
            # Old schema: (model_name, z_score, avg_wattage, runs)
            # New schema: (model_name, z_score, avg_quality, avg_joules, eval_runs, total_runs)
            logger.info("Migrating model_metrics from v1 schema to v2 (EMA format)...")
            ...
            # 2. Copy data from old columns → new columns
            #    avg_wattage → avg_joules, runs → total_runs
            #    avg_quality = z_score * avg_wattage (reverse from z = quality / joules)
            conn.execute("""
                UPDATE model_metrics
                SET avg_joules = CASE WHEN avg_wattage > 0 THEN avg_wattage ELSE 150.0 END,
                    total_runs = CASE WHEN runs > 0 THEN runs ELSE 0 END,
                    eval_runs  = CASE WHEN runs > 0 THEN runs ELSE 0 END,
                    avg_quality = CASE
                        WHEN avg_wattage > 0 THEN MIN(1.0, z_score * avg_wattage)
                        ELSE 0.5
                    END
            """)
            logger.info("Migration complete: copied avg_wattage→avg_joules, runs→total_runs, computed avg_quality.")

        elif not has_new:
            # Table was just created with new schema — nothing to migrate
            pass
