...
async def run_benchmark(model: str, suite: str = "full", ollama_url: str = None,
    ...
    if verbose:
        ...
        print(f"{'='*60}\n")
        print(f"  [⏳] Warming up model {model} (forcing Memory mapping)...")
    ...
    async with httpx.AsyncClient() as client:
        # Warmup ping to prevent 0-token timeouts on first run
        await infer(client, model, "Hi", ollama_url, temperature=0.1)
        if verbose:
